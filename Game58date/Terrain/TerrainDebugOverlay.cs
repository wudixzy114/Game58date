...
public sealed class TerrainDebugOverlay
{
    ...
    public void Draw(DebugTextSystem? debugText, TerrainRuntimeStats stats, TerrainGenerationSettings settings, TerrainViewMode viewMode)
    {
        ...
        debugText.Print($"Mode: {viewMode}", new Int2(20, 140), TextColor);
        debugText.Print($"View distance: {settings.ViewDistanceInChunks} chunks", new Int2(20, 164), TextColor);
        debugText.Print($"Build queue: {stats.QueuedBuildCount} queued / {stats.RunningBuildCount} running", new Int2(20, 188), TextColor);
        debugText.Print($"Last error: {stats.LastErrorMessage}", new Int2(20, 212), TextColor);
        debugText.Print("F5 toggles modes in-place, F6 resets current mode, F7 rebuilds terrain, F8 edits a debug block", new Int2(20, 236), HintColor);
    }
}
